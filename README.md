...
3. **Control what your agents can run**: Define which tools and packages your
   ...
   Update it once and all agents automatically get the change.
...
The gateway requires every browser origin to be declared explicitly. Repeat
`--allowed-web-origin` when running the gateway directly:

```sh
agentz gateway serve \
  --allowed-web-origin=https://agentz.example.com \
  --allowed-web-origin=http://localhost:3000
```

For the umbrella Helm chart, set the same origins under the gateway configuration:

```yaml
gateway:
  config:
    allowedWebOrigins:
      - https://agentz.example.com
```

Only absolute HTTP(S) origins are accepted. Paths, credentials, queries,
fragments, and wildcards fail gateway startup.
