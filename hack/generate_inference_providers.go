...
// Command generate-inference-providers pins OpenCode's provider catalog to the
// reviewed commit used by AgentZ.
package main
...
var excluded = map[string]string{
	"github-copilot": "oauth-only GitHub Copilot subscription",
...
func main() {
	...
	for id, provider := range providers {
		...
		if forced := providerKinds[id]; forced != "" {
			kinds[forced] = struct{}{}
		} else {
	...
	output.WriteString("// Code generated by hack/generate_inference_providers.go; DO NOT EDIT.\n\n")
	...
	finalOutput := string(formatted)
	const entriesPrefix = "var catalogEntries = []CatalogEntry{\n"
	const entriesSuffix = "var catalogNPMKinds = "
	rawStart := strings.Index(raw, entriesPrefix)
	rawEnd := strings.Index(raw, entriesSuffix)
	formattedStart := strings.Index(finalOutput, entriesPrefix)
	formattedEnd := strings.Index(finalOutput, entriesSuffix)
	if rawStart == -1 || rawEnd == -1 || formattedStart == -1 || formattedEnd == -1 {
		log.Fatal("catalogEntries block markers not found")
	}
	finalOutput = finalOutput[:formattedStart] + raw[rawStart:rawEnd] +
		finalOutput[formattedEnd:]

	if err := os.WriteFile(catalogOutput, []byte(finalOutput), 0o644); err != nil {
