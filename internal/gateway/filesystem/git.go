...
func (s *service) git(w http.ResponseWriter, r *http.Request) {
	...
	case gatewayapi.CodingGitDiscover, gatewayapi.CodingGitStatus,
		gatewayapi.CodingGitDiff, gatewayapi.CodingGitStashes:
		readOnly = true
	}
	if req.Prepare || !readOnly {
		s.mu.Lock()
		defer s.mu.Unlock()
	}
...
func (s *service) runGit(ctx context.Context, req GitRequest) (gatewayapi.CodingGitResult, error) {
	...
	run := func(cwd, index string, args ...string) (string, error) {
		...
		command.Env = []string{
			...
			"GIT_AUTHOR_EMAIL=stash@invalid",
			"GIT_COMMITTER_NAME=AgentZ",
			"GIT_COMMITTER_EMAIL=stash@invalid",
		}
		if index != "" {
	...
	if req.Git.Operation == gatewayapi.CodingGitDiscover {
		...
		for _, record := range strings.Split(raw, "\x00\x00") {
			...
			if err != nil {
				...
				entry.Reason = new("Checkout directory is missing")
				continue
			}
			rel, err = filepath.Rel(root, resolved)
			if err != nil || !filepath.IsLocal(rel) {
				entry.Available = false
				entry.Reason = new("Checkout leaves the project directory")
				continue
			}
			if !entry.Available {
				continue
			...
			if err != nil || actual != common {
				entry.Available = false
				entry.Reason = new("Checkout belongs to another repository")
			}
		}
		raw, err = run(
			repo,
	...
	if req.Prepare {
		...
		if _, err := os.Stat(filepath.Join(repo, ".git")); errors.Is(err, os.ErrNotExist) {
			_, err = run(root, "", "init", "--initial-branch="+req.BaseBranch, repo)
			if err != nil {
				return result, err
			}
		}
		...
		if headErr != nil {
			_, err = run(
				...
				"-B",
				req.BaseBranch,
				"refs/remotes/origin/"+req.BaseBranch,
			)
			if err != nil {
				return result, err
			}
		...
		if directory != repo {
			if _, err := os.Stat(directory); errors.Is(err, os.ErrNotExist) {
				_, err = run(repo, "", "check-ref-format", "--branch", req.Branch)
				if err != nil {
					return result, errors.New("invalid worktree branch")
				}

				_, err = run(repo, "", "worktree", "add", "-b", req.Branch, directory, base)
				if err != nil {
					return result, err
				}
			}
	...
	readStashes := func() ([]gatewayapi.CodingGitStash, error) {
		...
		for i := 0; i+3 < len(entries); i += 4 {
			...
			stashes = append(
				stashes,
				gatewayapi.CodingGitStash{
					Reference: strings.TrimSpace(entries[i]),
					Oid:       entries[i+1],
					CreatedAt: created,
					Message:   entries[i+3],
				},
	...
	readPatches := func() ([]gatewayapi.CodingGitPatch, error) {
		...
		switch {
		case req.Git.Stash != nil:
			args = append(args, *req.Git.Stash+"^1", *req.Git.Stash)
		case comparison == gatewayapi.CodingGitStaged:
			args = append(args, "--cached")
		case comparison == gatewayapi.CodingGitAll:
			base := result.Head
			if base == "" {
		...
		switch {
		case req.Git.Stash != nil:
			_, err = run(directory, "", "rev-parse", "--verify", *req.Git.Stash+"^3")
			if err == nil {
				args := []string{
					"--literal-pathspecs",
					"diff-tree",
		...
		case comparison != gatewayapi.CodingGitStaged:
			...
			default:
				...
				err = os.WriteFile(spec, []byte(strings.Join(untracked, "\x00")+"\x00"), 0o600)
				if err != nil {
					return nil, err
				}
				_, err = run(
		...
		for _, chunk := range chunks {
			...
			for chunk != "" {
				...
				patches = append(
					patches,
					gatewayapi.CodingGitPatch{
						Path:     name,
						Patch:    patch,
						Revision: fmt.Sprintf("%x", sha256.Sum256([]byte(patch))),
						Binary:   file.IsBinary,
						CanStageHunks: !file.IsBinary && !file.IsRename && !file.IsCopy &&
							file.NewMode == 0 && !file.IsDelete && text,
					},
				)
			}
		}
	...
	case gatewayapi.CodingGitStashApply, gatewayapi.CodingGitStashPop, gatewayapi.CodingGitStashDrop:
		...
		index := slices.IndexFunc(
			stashes,
			func(stash gatewayapi.CodingGitStash) bool { return stash.Oid == *req.Git.Stash },
		)
		if index < 0 {
	...
	case gatewayapi.CodingGitStage, gatewayapi.CodingGitUnstage:
		...
		expected := gatewayapi.CodingGitUnstaged
		if req.Git.Operation == gatewayapi.CodingGitUnstage {
			expected = gatewayapi.CodingGitStaged
		}
	...
	case gatewayapi.CodingGitImport, gatewayapi.CodingGitApplyCommit:
		...
		_, err = run(directory, "", "check-ref-format", "--branch", *req.Git.Ref)
		if err != nil {
			return result, errors.New("invalid branch")
		}
		if req.Git.Operation == gatewayapi.CodingGitImport {
			_, err = run(
				directory,
				"",
				"merge",
				"--ff-only",
				"refs/remotes/origin/"+*req.Git.Ref,
			)
			if err != nil {
				return result, err
			}
	...
	if result.Head != "" && req.BaseBranch != "" {
		...
		hasRemote := err == nil && result.Branch != ""
		if !hasRemote {
	...
	if req.Git.Operation == gatewayapi.CodingGitExport {
		...
		commit, err := run(
			...
			*result.Tree,
			...
			result.Head,
			"-m",
			"Staged tree transport",
		)
		if err != nil {
			return result, errors.New("could not export staged tree")
		}
		transportRef := "refs/agentz/export"
		_, err = run(directory, "", "update-ref", transportRef, strings.TrimSpace(commit))
		if err != nil {
			return result, err
		}
		defer run(directory, "", "update-ref", "-d", transportRef)
		_, err = run(directory, "", "bundle", "create", file.Name(), "--branches", transportRef)
		if err != nil {
			return result, err
		}
