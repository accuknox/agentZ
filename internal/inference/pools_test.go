...
func TestResolvePoolContract(t *testing.T) {
	...
	if len(definition.Warnings) != 1 ||
		definition.Warnings[0].Code != agentzv1alpha1.InferencePoolWarningMixedProtocols {
...
func TestRenderPoolBackend(t *testing.T) {
	...
	if groups[0].Providers[0].OpenAI == nil || groups[0].Providers[0].OpenAI.Model == nil ||
		*groups[0].Providers[0].OpenAI.Model != "gpt" {
		t.Fatalf("primary model override = %#v, want gpt", groups[0].Providers[0].OpenAI)
	}
	if groups[1].Providers[0].Anthropic == nil || groups[1].Providers[0].Anthropic.Model == nil ||
		*groups[1].Providers[0].Anthropic.Model != "claude" {
	...
	for i := range groups {
		...
		if string(*health.UnhealthyCondition) !=
			"response == null || response.code == 429 || response.code >= 500" {
			t.Fatalf("group %d unhealthy condition = %q", i, *health.UnhealthyCondition)
		}
		if health.Eviction.Duration == nil || health.Eviction.Duration.Duration != 60*time.Second ||
			health.Eviction.ConsecutiveFailures == nil || *health.Eviction.ConsecutiveFailures != 3 ||
			health.Eviction.RestoreHealth == nil || *health.Eviction.RestoreHealth != 100 {
...
func TestRenderSandboxPoolTarget(t *testing.T) {
	...
	if len(expressions) != 1 || string(expressions[0]) !=
		"default(json(request.body).model, \"\") in [\"pool\"]" {
