...
export function CodingWorkspace({
  ...
  const drag = useRef<{ x: number; width: number }>(null)
  ...
  const github = useQuery(
    queryOptions({
      ...
      enabled: open && tab === "github" && actor?.user.id !== undefined,
    })
  )
    ...
    <>
      ...
      >
        <div
          role="separator"
          tabIndex={0}
          aria-label="Resize coding workspace"
          aria-orientation="vertical"
          aria-valuemin={360}
          aria-valuemax={1000}
          aria-valuenow={width}
          className="hover:bg-primary/30 focus-visible:bg-primary/30 absolute inset-y-0 -left-1 z-20 hidden w-2 cursor-col-resize touch-none outline-none lg:block"
          onDoubleClick={() => setWidth(480)}
          onKeyDown={(event) => {
            if (event.key !== "ArrowLeft" && event.key !== "ArrowRight") return
            event.preventDefault()
            setWidth((value) =>
              Math.max(360, Math.min(1000, value + (event.key === "ArrowLeft" ? 24 : -24)))
            )
          }}
          onPointerDown={(event) => {
            drag.current = { x: event.clientX, width: panel.current?.clientWidth ?? width }
            event.currentTarget.setPointerCapture(event.pointerId)
          }}
          onPointerMove={(event) => {
            if (!drag.current) return
            setWidth(
              Math.max(360, Math.min(1000, drag.current.width + drag.current.x - event.clientX))
            )
          }}
          onPointerUp={() => {
            drag.current = null
              ...
              <>
                <div className="flex items-center gap-2 border-b px-3 py-2">
                  ...
                  <select
                    ...
                    disabled={pending || data.files.length > 0}
                    className="bg-background min-w-0 flex-1 rounded text-xs"
                    onChange={(event) => {
                  ...
                  onSubmit={(event) => {
                    ...
                    startTransition(async () => {
                      try {
                        ...
                        toast.error(
                          error instanceof Error ? error.message : "Could not commit staged changes"
                        )
                      }
                    })
                  }}
                >
                  ...
                  <Button
                    type="submit"
                    size="sm"
                    disabled={pending || !message.trim() || stagedFiles.length === 0}
                  >
                ...
                {data.files.length > 0 ? (
                  <>
                    ...
                    (!selected ||
                      ...
                      !(staged ? stagedFiles : changedFiles).some(
