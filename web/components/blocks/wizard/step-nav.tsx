      ...
      {steps.map((step, index) => {
        ...
        return (
          ...
          >
            ...
            <div className={cn("flex justify-center", layout === "horizontal" && "flex-1")}>
              {!isLast && (
                <div
                  aria-hidden="true"
                  data-status={status}
                  className={cn(
                    "bg-muted transition-all duration-300 ease-in-out data-[status=success]:bg-primary data-disabled:opacity-50",
                    layout === "vertical" ? "hidden h-full w-0.5 md:block" : "h-0.5 min-w-4 flex-1"
                  )}
                />
              )}
            </div>
