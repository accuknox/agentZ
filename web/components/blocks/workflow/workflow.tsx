        ...
        <Panel position="top-left" className="w-sm max-w-sm p-0">
          <Collapsible defaultOpen={false} className="group/workflow-summary">
            <CollapsibleTrigger asChild>
              <button
                type="button"
                className="flex w-full items-start gap-2 px-3 py-2 text-left outline-hidden transition-colors hover:bg-muted/30 focus-visible:ring-2 focus-visible:ring-ring/60"
                aria-label="Toggle workflow summary"
              >
                <div className="min-w-0 flex-1">
                  <h2 className="truncate text-sm font-medium">{workflow.title}</h2>
                </div>
                <CornerDownLeftIcon className="mt-0.5 size-4 shrink-0 text-muted-foreground" />
              </button>
            </CollapsibleTrigger>
...
function WorkflowCanvasNodeCard({ data, selected }: FlowNodeProps<WorkflowCanvasNode>) {
  return (
    <Node handles={data.handles} className={selected ? "ring-2 ring-primary/60" : undefined}>
      <NodeHeader>
      ...
      <div className="flex flex-wrap gap-2">
        {tools.length > 0 ? (
          <>
            {compact ? (
              <span
                className="inline-flex h-5 items-center text-muted-foreground"
          ...
          <span className="text-sm text-muted-foreground">No preferred tools</span>
        )}
      </div>
    </div>
  )
