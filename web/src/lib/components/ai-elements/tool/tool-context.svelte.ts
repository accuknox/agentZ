...
export class ToolClass {
	...
	input = $state<ToolSchema['input']>(undefined);
	output = $state<ToolSchema['output']>(undefined);
	...
	setOutput(output: ToolSchema['output']) {
		this.output = output;
...
export function getToolContext(): ToolClass {
	const context = getContext<ToolClass>(TOOL_CONTEXT_KEY);
	if (!context) {
